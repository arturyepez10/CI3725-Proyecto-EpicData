...
def validate_vardef(ast: AST.SymDef, sym_table: dict) -> AST.AST:
    ...
    # Se validan los tipos de las variables
    return validate_type(ast.value, sym_table)

def validate_assign(ast: AST.Assign, sym_table: dict) -> AST.AST:
...
def validate_expr(ast: AST.AST, sym_table: dict) -> AST.AST:
...
def validate_type(ast: AST.AST, sym_table: dict) -> AST.AST:
    # Se valida que la expresión sea de tipo booleano
    if ast.type != 'bool':
        raise Exception(f'Expresión no es de tipo booleano')
    else:
