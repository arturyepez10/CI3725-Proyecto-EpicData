...
test_sol.extend(list((SymDef(Type(varType), Id("x"), Comparison(comparison, NumberBinOp(plus_or_minus, NumberUnOp(plus_or_minus_unary, Number(2)), Number(3)), NumberBinOp(prod_div_pow, Number(2), NumberUnOp(plus_or_minus_unary, Number(5))) )) for plus_or_minus_unary in NUM_UN_OPS for plus_or_minus in NUM_UN_OPS for comparison in COMPARISONS for prod_div_pow in ["^", "*", "/"] for varType in VAR_TYPE)))
...
test_sol.extend(list((AssignArray(Id('array'), ElemList(None).__debug_Init__([NumberBinOp(binNumOp, Number(2), NumberUnOp(unOp, Number(3))), BooleanBinOp (bool_bin_op, Comparison(comparison, Id('z'), Boolean('false')), Id('x') ), ArrayAccess(Id('h'),NumberBinOp(binNumOp, Number(2), NumberUnOp(unOp, Number(3))) )])) for binNumOp in NUM_BIN_OPS for unOp in NUM_UN_OPS for comparison in COMPARISONS for bool_bin_op in BOOL_BIN_OPS)))
...
test_sol.extend(list((SymDef(Type(TypeArray(Type(arr_type))), Id('array'), ElemList(None).__debug_Init__([NumberBinOp(binNumOp, Number(2), NumberUnOp(unOp, Number(3))), BooleanBinOp (bool_bin_op, Comparison(comparison, Id('z'), Boolean('false')), Id('x') ), ArrayAccess(Id('h'),NumberBinOp(binNumOp, Number(2), NumberUnOp(unOp, Number(3))) )])) for binNumOp in NUM_BIN_OPS for unOp in NUM_UN_OPS for comparison in COMPARISONS for bool_bin_op in BOOL_BIN_OPS for arr_type in VAR_TYPE)))
