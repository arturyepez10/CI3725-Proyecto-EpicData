...
test_sol.extend(list((Assign(Id('array'), Array([BinOp(bin_num_op, Number(2), UnOp(un_op, Number(3))), BinOp (bool_bin_op, Comparison(comparison, Id('z'), Boolean(False)), Id('x') ), ArrayAccess(Id('h'),BinOp(bin_num_op, Number(2), UnOp(un_op, Number(3))) )])) for bin_num_op in NUM_BIN_OPS for un_op in NUM_UN_OPS for comparison in COMPARISONS for bool_bin_op in BOOL_BIN_OPS)))
...
test_sol.extend(list((SymDef(Type(TypeArray(PrimitiveType(arr_type))), Id('array'), Array([BinOp(bin_num_op, Number(2), UnOp(un_op, Number(3))), BinOp (bool_bin_op, Comparison(comparison, Id('z'), Boolean(False)), Id('x') ), ArrayAccess(Id('h'),BinOp(bin_num_op, Number(2), UnOp(un_op, Number(3))) )])) for bin_num_op in NUM_BIN_OPS for un_op in NUM_UN_OPS for comparison in COMPARISONS for bool_bin_op in BOOL_BIN_OPS for arr_type in VAR_TYPE)))
...
test_sol.extend(list((SymDef(Type(TypeArray(PrimitiveType(arr_type))), Id('array'), Array([BinOp(bin_num_op, Number(2), BinOp(bin_num_op, UnOp(un_op, Number(3)), Number(7))), BinOp (bool_bin_op, Comparison(comparison, Id('z'), Boolean(False)), Function(Id('fun'), [ArrayAccess(Id('h'), Id('x'))])), ArrayAccess(Id('h'),BinOp(bin_num_op, Number(2), Quoted(UnOp(un_op, Number(3)))) )])) for bin_num_op in NUM_BIN_OPS for un_op in NUM_UN_OPS for comparison in COMPARISONS for bool_bin_op in BOOL_BIN_OPS for arr_type in VAR_TYPE)))

# Arreglo dentro de funcion con arreglos que es accesada como un arreglo
test_cases.append("Hola([1, 2], 2 + 2, h(2), [true, false][1])[1]")
test_sol.append(ArrayAccess(Function(Id("Hola"), [Array([Number(1), Number(2)]), BinOp('+', Number(2), Number(2)), Function(Id("h"), [Number(2)]), ArrayAccess(Array([Boolean(True), Boolean(False)]), Number(1))]), Number(1)))
